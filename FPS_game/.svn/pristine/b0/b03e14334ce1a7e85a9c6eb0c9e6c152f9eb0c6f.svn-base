...
void Car::setMesh(MyMesh _mesh){
	this->mesh = _mesh;
}

void Car::setPosition(glm::vec3 _position){
...
void Car::InitCarUsingGlmModel(){
	//use MyMesh to read the .obj, textures and shaders files
	this->mesh.reader.init("external_files/car/car-n.obj","shaders/vl.glsl","shaders/fl.glsl");
	
	
}

void Car::draw(glm::mat4 model, glm::mat4 view,glm::mat4 projection, glm::vec3 scale, float alpha, glm::vec3 color){
	...
	model = glm::scale(model,glm::vec3(1.5,1.5,1.5));//rezise the car
...
void Car::setPositionCoordinate(int index, GLfloat coordinateValue){
	if(index == 0){
		this->position.x = coordinateValue;
	}
	
	if(index == 1){
		this->position.y = coordinateValue;
	}
	
	if(index == 2){
		this->position.z = coordinateValue;
	}
